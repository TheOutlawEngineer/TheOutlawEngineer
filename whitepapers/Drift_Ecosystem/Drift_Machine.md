...
This paper is about that **drift** — the quiet, cumulative, operationally convenient decisions that turn SCADA systems into attack surfaces. Not because anyone intended harm, but because industrial environments reward shortcuts and punish any friction. The result is a landscape where improvised connectivity become invisible infrastructure.

And invisible infrastructure is dangerous kind.
...
The people who keep these systems alive aren’t sitting in climate‑controlled SOCs with dashboards and alerts. They’re in the extreme heat or cold crawling behind ancient conveyors, climbing corrosian pitted catwalks, and trying to reseat a card in a cabinet that hasn’t been opened since at least the first Bush administration. They don’t have time to file tickets, escalate or wait for approvals. They need the line running now. So they do what works: patch, bypass, improvise, repeat.
...
And once a shortcut works, it stays, it becomes normal. Once it’s normal, it becomes invisible. Just anothe brick in the wall.

Invisible infrastructure is the enemy of security. It’s the access point nobody remembers installing, the vendor VPN nobody disabled, the “temporary” remote connection that became permanent because the next week turned into the next quarter which turned into next year. These shortcuts accumulate quietly, layer by layer, until the control network looks nothing like the P&ID diagram taped inside the maintenance office.
...
None of these actions are malicious. They’re practical. They’re born of survival. They’re what keeps production moving. Uptime. Industrial systems have long memories and short documentation cycles. What was meant to be temporary becomes invisible the moment that crisis ends. As one fire is extinguished another starts to smolder.
...
A PLC set up just for testing becomes an internet‑facing endpoint and is  indexed by Shodan within hours.

And because nobody updates the network diagram, nobody knows these things exist. The diagram taped inside IT office shows a clean, segmented, orderly system. Corrective actions on proper documentation are issued, but the gap between the reality of the floor and the perception  of the front office guarantees eventual non-compliance. The actual network is a patchwork of improvisations layered over years of production pressure.
...
None of these actions are catastrophic on their own. They’re shortcuts — the same kind that keep plants alive at 3:00 a.m. But shortcuts accumulate. And once the crisis ends, nobody circles back to undo them. The vendor leaves. The engineer goes home. The releaved plant manager forgets the conversation. The documentation never gets updated. The event never happened. 
...
A vendor tunnel stays active because nobody remembered or had the time to disable it after commissioning.  
...
Industrial protocols were designed for reliability, not security. They assume trusted networks and trusted operators. They lack encryption, authentication, and integrity checks. An attacker who can speak the protocol can:
...
Even modern protocols designed with cryptography — like OPC UA — fall victim to operational drift. Proper Public Key Infrastructure management is an administrative nuisance, engineers routinely configure systems to auto‑accept certificates using Trust‑On‑First‑Use logic or rely on default self‑signed certs just to get data flowing. Sure, the crypto exists on paper, but the operational shortcut renders it vulnerable to manipulation.
...
- unencrypted MQTT brokers on port 1883  
- insecure WebSockets  
- default administrative web interfaces  

They create an outbound‑initiated tunnel that acts as an unauthenticated inbound point for anyone scanning the broker’s public‑facing endpoint.

### 5.5 Firmware Vulnerabilities

Legacy PLCs often run firmware that hasn’t been updated in years. Some can’t be updated at all without replacing hardware. Attackers exploit:

- buffer overflows  
- insecure update mechanisms  
- unsigned firmware  
- exposed debug interfaces  
- outdated web servers baked into the controller  
...
Attackers don’t need a reason to target your plant. They target exposed devices simply because they’re exposed. Motivations vary:
...
Once an attacker reaches a PLC or an exposed industrial edge device, the mechanics of exploitation are not theoretical. They are practical, predictable, and shaped by how industrial control systems actually behave. Hollywood imagines explosions and dramatic sabotage. Real attackers aim for subtlety — because subtlety is harder to detect, easier to maintain, and far more damaging.
...
Spoofed sensor values can:

- mask overheating  
- hide vibration anomalies  
- fake flow rates  
- simulate normal pressure  
- conceal bearing wear  
- delay maintenance triggers  

A PLC trusts its sensors. A SCADA system trusts the PLC. The plant trusts SCADA. Thats the chain. A single spoofed value cascades upward until the entire system believes its legitimate.
...
Poisoned data leads to:

- false maintenance schedules  
- incorrect trend analysis  
- misdiagnosed failures  
- bad predictive models  
...
- alter graphics  
- change tag values  
- disconnect operator stations  
- freeze screens  
- desynchronize displays  

Operators trust SCADA. If SCADA lies, operators will act on the lie.

### 6.7 Safety System Interaction

Safety controllers (SIS) are designed to be isolated, but drift often erodes that isolation. If an attacker reaches the safety network, they can:
...
Every hop increases the control‑plane.
...
- modifying logic  
- adding hidden rungs  
- creating rogue tasks  
- installing vendor‑like remote agents  
- abusing cloud connectors  
- leaving backdoor credentials  
...
Attackers don’t need to destroy machinery. They just need to create uncertainty. Enough controlled chaos to make operations second-guess.
...
Production losses look like inefficiency, not sabotage.
...
The documented system is incorrect,  a fantasy. The real system is a living organism shaped by fatigue, pressure, shortcuts, and necessity. Uptime is king. Drift wins because drift is continuous. Audits lose because audits are episodic.
...
Understanding drift is not enough. Recognizing shortcuts is not enough. Mapping exposure is not enough. Industrial security only becomes real when the plant changes how it operates. 

Not in theory, in practice. 

Implementation is the difference between awareness and security.

### 9.1 Eliminate Invisible Infrastructure

The first step is finding everything in the drift:

- every vendor tunnel  
- every forgotten firewall rule  
- every Wi‑Fi access point  
- every IIoT gateway  
- every cloud connector  
- every PLC with a public IP  
- every device installed during a crisis and never removed  
...
Segmentation must reflect the real network, not the diagram taped inside the maintenance office. Trust boundaries must be built around what exists, not what was intended. That means:

- isolating PLCs  
- isolating HMIs  
- isolating SCADA servers  
- isolating historians  
- isolating engineering workstations  
- isolating safety controllers  
- isolating IIoT gateways and cloud connectors  
...
Plants must stop building architecture during emergencies. Crisis changes should be:

- documented  
- reviewed  
- reversed if temporary  
- rebuilt properly if permanent  

Emergency fixes should not become infrastructure. Temporary  is temporary. 

### 9.5 Establish Real Change Control

Change control must exist outside policy documentation. It must be enforced.  Even when production precievess it as useless and a nuisance. That means:
...
If production is allowed to override change control, drift wins.
...
- inspect every PLC  
- inspect every firewall  
- inspect every switch  
- inspect every IIoT gateway  
- inspect every cloud connector  
- validate segmentation physically  
- validate access paths practically  

Audits measure reality, not intent.

### 9.7 Build Security Into Daily Operations

Security cannot be an annual event. It must be:
...
Security must survive production pressure. If it doesn’t, it fails.
...
Industrial systems evolve daily.
...
Implementation is the moment the plant decides to see reality.
